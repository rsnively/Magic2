...
public class ControlPanelView extends View {
    ...
    private String GetDialogString(Phase p) {
        if (Game.Get().CostsBeingPaid())
            return "Pay " + Game.Get().GetCostBeingPaid().GetString();
        switch (p.GetName()) {
            case Upkeep:
            case Untap:
            case Draw:
    ...
    protected void Render(Graphics2D g) {
        ...
        g.drawString("P1 Cards in Exile: " + Game.Get().GetPlayer1().GetExile().size(), GetRect().GetLeft(), GetRect().GetTop() + 140);

        g.drawString("Player 2", GetRect().GetLeft(), GetRect().GetTop() + 180);
        g.drawString("========", GetRect().GetLeft(), GetRect().GetTop() + 200);
        g.drawString("P2 Life: " + Game.Get().GetPlayer2().GetLifeTotal(), GetRect().GetLeft(), GetRect().GetTop() + 220);
        g.drawString("P2 Cards in Hand: " + Game.Get().GetPlayer2().GetHand().size(), GetRect().GetLeft(), GetRect().GetTop() + 240);
        g.drawString("P2 Cards in Library: " + Game.Get().GetPlayer2().GetLibrary().size(), GetRect().GetLeft(), GetRect().GetTop() + 260);
